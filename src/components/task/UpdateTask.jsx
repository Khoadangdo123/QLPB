...
const UpdateTask = ({ open, setOpen, phanDuAn, congViecCha, duAn, task }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  useEffect(() => {
    if (task) {
      ...
      // Load other task-related data if necessary
    }
  }, [task, reset]);
  ...
  const submitHandler = async (data) => {
    let CongViec = {
      ...
      trangThaiCongViec: task ? task.trangThaiCongViec : false, // Keep status for update
